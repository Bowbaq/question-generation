...
public class TestWhPhraseGenerator extends TestCase {
     ...
     * parse = AnalysisUtilities.getInstance().parseSentence(sentence).getTree();
     * res = TestQuestions.getQuestionOutputStringsFromParse(qt, parse);
     * assertFalse(res.toString(),res.contains("What facilities are the improvements related to?") ||
     * res.contains("Which facilities are the improvements related to?"));
     ...
     * parse = AnalysisUtilities.getInstance().parseSentence(sentence).getTree();
     * res = TestQuestions.getQuestionOutputStringsFromParse(qt, parse);
     * assertFalse(res.toString(),res.contains(
     * "What kind of NN did Los Angeles become known as'' for its role in supplying beef and other foodstuffs to hungry miners in the north?"));
     * }
     */
